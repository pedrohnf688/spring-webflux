...
public class TaskCustomRepository {
    ...
    public TaskCustomRepository(ReactiveMongoOperations operations, ReactiveMongoTemplate reactiveMongoTemplate) {
        this.operations = operations;
        this.reactiveMongoTemplate = reactiveMongoTemplate;
    }


    public Mono<Long> updateStateToDoneForOlderTaks(LocalDate date) {
        return reactiveMongoTemplate.updateMulti(
                Query.query(Criteria.where("created").lt(date).and("state").is(TaskState.DOING)),
    ...
    private Mono<Page<Task>> execute(Task task, Example<Task> example, Pageable pageable) {
        return query(example, pageable, task)
                .flatMap(query -> operations.find(query, Task.class).collectList())
    ...
    private Mono<Page<Task>> paginate(List<Task> tasks, Pageable pageable, Example<Task> example) {
        return Mono.just(tasks)
                .flatMap(it -> operations.count(Query.query(Criteria.byExample(example)), Task.class))
    ...
    private Mono<Query> query(Example<Task> example, Pageable pageable, Task task){
        return Mono.just(example)
                .map(it -> Query.query(Criteria.byExample(example)).with(pageable))
